...
script_version_number(300) -- АТСАСИ У ТРАКТАРИСТА
	...
	function customServWindow()
		...
		imgui.Begin(u8'Поддерживаемые сервера', customServ_window, 2)
			imgui.BeginChild('##serverslistedit', imgui.ImVec2(535, 760), true) 
				imgui.Text(u8"Оффицально поддерживаемые сервера:") imgui.SameLine(409) 
				if imgui.Button(u8"Синхронизировать") then
					...
					lua_thread.create(function()
						downloadUrlToFile("https://api.kscripts.ru/getlistservers.php", "moonloader/config/KopnevScripts/servers.ctool", function(id, status, p1, p2)
							if status == dlstatus.STATUS_ENDDOWNLOADDATA then
								if status == dlstatus.STATUS_DOWNLOADINGDATA then
									SCM(string.format('Загружаем %d байт из %d байт', p1, p2))
								elseif status == dlstatus.STATUS_ENDDOWNLOADDATA then
	...
	--lua_thread.create(function() 
		...
		for i, v in ipairs(accounts) do
			...
			--wait(50)
				...
				for i4 in ipairs(ipserv) do
					--print(ipserv[i4]['domen'])
					if ip == ipserv[i4]['ip'] then ip2 = ipserv[i4]['domen'] break end
				end
				if (v['server_ip'] == ip2 or v['server_ip'] == ip) and v['server_port'] == port.."" and v['user_name'] == name then
					print('{00FF00}Успешно{FFFFFF} загружен аккаунт {2980b9}'..v['user_name']..'{FFFFFF}.')
					account_info = v
					break
				end
	...
	for i, v in ipairs(serv) do
		for i2, v2 in ipairs(v['list']) do
			for s in ipairs(chars) do
				if v2['ip']:find(chars[s]) then
					local a = {
						domen = v2['ip'],
						ip = ""
					}
					table.insert(ipserv, a)
					break
				end
			end
		end
	end
	async_http_request('https://api.kscripts.ru/getip.php', "servers="..toSendGet(encodeJson(ipserv)), function(res)
        if res then
            if res:sub(1,1) == '[' then
				ipserv = decodeJson(res)
				--print(encodeJson(ipserv))
            else sampAddChatMessage("[Connect Tool] Внутренняя ошибка. Попробуйте позже", 0xFF0000) print(res) end
        end
    end)
end
